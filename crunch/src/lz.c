...
lz_result_t lz_parse(const refs_t *refs, uint32_t max_cost, uint32_t num_fixed_offset_bits, arena_t *arena, arena_t scratch) {
    ...
    token_slice_t tokens = token_slice_make(&scratch, refs_num(refs));
    ...
    for (uint32_t i = refs_num(refs); i-- > 0;) {
        ...
        token_view_t possible_tokens = refs_get_tokens(refs, i);
        for (uint32_t j = 0; j < possible_tokens.num; j++) {
            token_t t = token_view_get(possible_tokens, j);

        }
    }

    // Now build the final token stream by walking the token list from the first element
    token_array_t token_stream = token_array_make(arena, refs_num(refs));
    for (uint32_t i = 0; i < tokens.num; i += token_slice_get(tokens, i).length_minus_one + 1) {
        token_array_add(&token_stream, token_slice_get(tokens, i), arena);
    }

    return (lz_result_t) {
        .tokens = token_view_from_array(&token_stream),
